...
This blog post is a comprehensive guide to using Terraform to create a complex three-tier architecture on AWS. Everything is `#opensource` so feel free to leverage.It servers as a strong prescription around Terraform modules to provide encapsulation and abstraction & real-world best practices to create complex Cloud infrastructure. It is written to help people at all levels and roles, from junior developers to CxOs, to learn about best practices and ways to effectively use IaC in a Digital Transformation journey. It would also serve as a valuable resource for architects and project managers looking to impress existing customers with their knowledge of IaC, automation-first and security-first approaches. They can leverage the blog insights to supercharge their projects and client engagements. The objective insights and real-world best practices mentioned in the blog can also be directly put in proposals/RFP's. The automation-first, security-first approach will elevate the proposals and create a positive enviornment. 
